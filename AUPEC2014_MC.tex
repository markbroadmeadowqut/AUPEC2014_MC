...
\section{Implementation of power and signal isolation using PCB transformers}
	...
	%TODO: NOTE: The datasheet states 24mA at 3V on the front page, but doesn't mention it again in the Abs max ratings or reccommended ratings (it is implied in some test conditions later on though).  Thus I think the part is limited by the power dissipation and the 50mA max in/out current and thus will be OK in this application.  I am worried, however, that saying its 24mA @ 3V will look inadequate (given the input current data in later figures shows >24mA for almost all frequencies except right on the MIF).  Feel free to change to whatever you think is best...
	% The logic family is UHS - ultra high speed.  Device is Fairchild NC7WZ14 as per schematic.
	% Datasheet: http://www.fairchildsemi.com/ds/NC/NC7WZ14.pdf
