...
A two-winding coreless PCB transformer may be described using the high-frequency transformer model in \figurename{2} \cite{TangHuiFundamental}, where:
\begin{itemize}
\item[] \hspace*{-3em} $ R_{1} $ Primary winding resistance;
\item[] \hspace*{-3em} $ R_{2}' $ Secondary winding resistance, referred to primary;
\end{itemize}
...
%
\begin{equation}
	M_{ij} = \frac{\mu_{0}\pi}{h_{1}\ln(\frac{r_{2}}{r_{1}})h_{2}\ln(\frac{a_{2}}{a_{1}})}\int\limits_{0}^{\infty}{S(kr_{2},kr_{1})S(ka_{2},ka_{1})Q(kh_{1},kh_{2})\exp^{-k \mod{z}}dk}
\end{equation}
...
\section{Topology}
	...
	\subsection{Clock Generation and Recovery} % Using method: gating of drive signal (could also be: divide drive signal by two; OR: Amplitude modulation)
	In order to provide a clock signal of approximately 8MHz to the frontend ADC, a method of transferring this clock signal via the PCB transformer from the DAQM's primary side to the secondary side (where the sigma-delta data converter resides) was required.  Due to the minimal secondary power required by the DAQM's frontend, the sigma-delta ADC's clock signal may be encoded simply by gating the transformer's primary drive current.  Although the DAQM's clock signal is at a much higher frequency than the signals used in isolated gate drive systmes, the modulation principle is similar \cite{TangHuiFundamental}.  [FIGURE] shows a standard silicon diode used to gate the relaxation oscillator such that an external clock signal of arbitarary timing may 100\% amplitude-modulate the transformer's primary drive current.
	To recover the clock signal, the transformer's secondary output is essentially low-pass filtered with a cutoff frequency approximately equal to the [SOMETHING].  [FIGURE] shows the decoding circuit, using a Schmitt trigger inverter (with feedback) to bias the filtered signal and provide clean clock transistions for the data converter's clock input.  In applications requiring greater secondary output power, a modulation depth of less than 100\% could be achieved using a transistor and parallel resistor in the primary winding's current path, athough it can be expected that will require a more complex clock recovery filter.
	
	\subsection{Data Recovery}
	The sigma-delta data converter outputs both voltage and current infomration on a single manchester-encoded bitstream at a rate equal to $ f_{clock}/4 $ (typically 2MHz).  To transfer this data from the transformer's secondary side back to the primary, it is necessary to effect the transformer in some way so that the disturbance is detectable at the primary.  Such disturbance detection is performed in commercial RFID systems, where the receiver modulates its antenna's resonant frequency to allow the transmitter to identify the receiving device \cite{RFID}.
...
To validate the use of planar coreless PCB transformers in the data acquisition module design, a test PCB transformer was manufactured on a standard two-layer, 1.6mm PCB.  The test transformer had 11 turns with identical primary and secondary windings.  The track and spacing width was equal at $ 203.2\mu $m (8 mil), giving an outer diameter of 13.7mm.
	...
	\subsection{Power}
	To approximate the expected load of the sigma-delta modulator and associated secondary circuitry, a $500\Omega $ load resistor was fitted after the rectifier.  By sweeping the primary drive frequency over the range of 500kHz to 25MHz, the coreless PCB test transformer, with primary-side Schmitt trigger driver and secondary side rectifier could be analysed for input impedance- and output voltage- versus frequency (FIGURE).  The figure shows a clear distinction between the MIF (13MHz) and MEF (15.5MHz), however the reasonably low quality factor of the MEF peak suggests that driving the transformer at a frequency of compromise between the MIF and MEF may be desirable.  The also data demonstrates the effectiveness of driving the transformer in a bipolar manner, which has resulted in an output voltage approximately 1.6 times the 3.3V primary supply voltage - thus increasing the usable voltage range of the data acquisition module and simplifying the secondary voltage supply circuitry.
